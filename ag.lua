...
local CONFIG = {
  ...
  server_url = "ws://192.168.0.10:8765/cc",
  auto_discover = true,
  reconnect_delay = 3,
...
local function try_connect(url, timeout)
  local ok, ws_or_err = pcall(function()
    if timeout then
  ...
  if not ok and timeout then
    local ok2, ws_or_err2 = pcall(function() return http.websocket(url) end)
...
local function discover_server()
  ...
  local batch_size = cfg.batch_size or 24
  ...
  while index <= #candidates do
    ...
    local found_url = nil
    local tasks = {}
    local last = math.min(index + batch_size - 1, #candidates)
    for i = index, last do
      local url = candidates[i]
      tasks[#tasks + 1] = function()
        if found_ws then return end
        local ws_try = try_connect(url, timeout)
        if ws_try then
          if not found_ws then
            found_ws = ws_try
            found_url = url
          else
            pcall(function() ws_try.close() end)
          end
        end
      end
    end
    ...
    if found_ws then
      log_line("Discovered server: " .. found_url)
      return found_ws, found_url
    end
    index = last + 1
  end
  return nil
...
local function connect()
  while true do
    ...
    for _, url in ipairs(tried) do
      ...
      local result = try_connect(url, 3)
      if result then
        ws = result
